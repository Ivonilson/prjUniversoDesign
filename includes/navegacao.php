		...
		<ul class="navbar-nav ml-auto col">
			...
			<li>
				<a href="/?pagina=logout" class="nav-link text-light col-2">
					<i class="fa fa-sign-out float-right" title="Sair do Sistema"><span style="font-size: 15px; font-family: Arial, sans-serif">Logout</span></i>
				</a>
			</li>
		</ul>
		...
		<div class="modal-content">
			...
			?>
				...
				<div class="modal-body">
					...
					<span class="small text-danger">Cadastrado em: </span>
					<p class="text-muted"> <?= date_format(date_create($dados->data_cadastro), 'd/m/Y H:i:s')
											?></p>
					<span class="small text-danger">Cadastrado por:</span>
					<p class="text-muted"> <?= $dados->usuario
											?></p>

					<span class="small text-danger">Remetente: </span>
					<p class="text-muted"><?= $dados->remetente ?></p>

					<span class="small text-danger">Destinatário: </span>
					<p class="text-muted"><?= $dados->destinatario ?></p>

					<span class="small text-danger">Meio da notificação: </span>
					<p class="text-muted"><?= $dados->meio_notificacao ?></p>

					<!--<span class="small text-danger">Data de emissão: </span><p class="text-muted"> <?= date_format(date_create($dados->data_emissao), 'd/m/Y')
																										?></p>-->

					<span class="text-danger font-weight-bold">Data limite: </span>
					<p class="text-muted font-weight-bold"> <?= date_format(date_create($dados->data_limite), 'd/m/Y')
															?></p>

					<span class="small text-danger">Prioridade: </span>
					<p class="text-muted"><?= $dados->prioridade ?></p>

					<span class="small text-danger">Observações: </span>
					<p class="text-muted"><?= $dados->observacoes ?></p>
