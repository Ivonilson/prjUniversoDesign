...
class OsDoDia {
	...
	public function dadosDoDia()
	{
		...
		$querySelect = "SELECT tbl_os.cod_os AS cod_os, tbl_orcamento.id_orcamento AS numeroOrcamento, tbl_cliente.nome AS nome, tbl_os.data_cadastro AS data_cadastro, tbl_os.data_agendamento AS data_agendamento, tbl_os.sit_pagamento AS sit_pagamento, tbl_os.status AS status, tbl_os.observacao AS observacao FROM tbl_os INNER JOIN tbl_orcamento ON tbl_os.id_orcamento = tbl_orcamento.id_orcamento INNER JOIN tbl_cliente ON tbl_orcamento.id_cliente = tbl_cliente.id_cliente ORDER BY tbl_orcamento.id_orcamento";
